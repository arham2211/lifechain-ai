  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const visitsResponse = await visitService.getVisits({ patient_id: patientId });
        const reportsResponse = await labService.getLabReports({ patient_id: patientId });
        const pendingReports = reportsResponse.items.filter((r) => r.status === 'pending');
      ...
      <div className="space-y-8">
        <section className={`${glassCard} p-6 lg:p-10`}>
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-slate-500 font-semibold">Welcome back</p>
              <h2 className="text-4xl font-bold mt-2 text-slate-900">{user?.name || 'Patient'}</h2>
              <p className="text-slate-600 mt-3 max-w-2xl">
                Your personalized health hub with AI insights, lab intelligence, and actionable care recommendations.
              </p>
            </div>
            <div className="px-6 py-4 rounded-2xl bg-slate-50 border border-slate-200">
              <p className="text-xs uppercase tracking-[0.3em] text-slate-500 font-semibold">Patient ID</p>
              <p className="text-lg font-semibold text-slate-900 mt-1">{user?.entity_id}</p>
            </div>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard title="Total Visits" value={stats.totalVisits} icon={Calendar} color="primary" />
          <StatCard title="Lab Reports" value={stats.totalReports} icon={FileText} color="success" />
          <StatCard title="Pending Reports" value={stats.pendingReports} icon={Activity} color="warning" />
        </div>

        {stats.recentRecommendations.length > 0 && (
          <section className={`${glassCard} p-6`}>
            <h3 className="text-2xl font-semibold mb-4 text-slate-900">AI Health Recommendations</h3>
              ...
              {stats.recentRecommendations.map((rec, index) => (
                <div key={index} className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="text-xs uppercase tracking-[0.3em] text-slate-500 font-semibold">Condition</p>
                      <h4 className="text-lg font-semibold text-slate-900">{rec.disease_name}</h4>
                    </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className={`${glassCard} p-6`}>
            <h3 className="text-xl font-semibold mb-4 text-slate-900">Health Snapshot</h3>
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: 'Upcoming Appointments', value: Math.max(1, Math.round(stats.totalVisits / 4)) },
                ...
                <div key={item.label} className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  <p className="text-xs text-slate-500 uppercase tracking-[0.3em] font-semibold">{item.label}</p>
                  <p className="text-3xl font-semibold mt-2 text-slate-900">{item.value}</p>
                </div>
              ))}
            </div>
          </div>
