      ...
      <div className="space-y-8 text-white">
        <section className={`${glassCard} p-6 lg:p-10`}>
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              ...
              <h2 className="text-4xl font-bold mt-2">{user?.name || 'Patient'}</h2>
            ...
            <div className="px-6 py-4 rounded-2xl bg-white/5 border border-white/10">
              <p className="text-xs uppercase tracking-[0.3em] text-white/60">Patient ID</p>
              <p className="text-lg font-semibold text-white mt-1">{user?.entity_id}</p>
            </div>
          </div>
        </section>
        ...
        {stats.recentRecommendations.length > 0 && (
          <section className={`${glassCard} p-6`}>
            <h3 className="text-2xl font-semibold mb-4">AI Health Recommendations</h3>
              ...
              {stats.recentRecommendations.map((rec, index) => (
                <div key={index} className="p-4 rounded-2xl bg-white/5 border border-white/10">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="text-xs uppercase tracking-[0.3em] text-white/60">Condition</p>
                      <h4 className="text-lg font-semibold text-white">{rec.disease_name}</h4>
                    </div>
                  ...
                  <div className="space-y-3">
                    {rec.recommendations.slice(0, 3).map((r, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <p className="text-sm text-white/80">{r.recommendation}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className={`${glassCard} p-6`}>
            <h3 className="text-xl font-semibold mb-4">Health Snapshot</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <a
                  key={action.title}
                  href={action.href}
                  className="group p-4 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all flex items-start gap-3"
                >
